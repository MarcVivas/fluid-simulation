...
pub struct DensityErrorCompute {
    density_error_compute_pass: ComputePass,
    #[allow(unused)]
    density_error_compute_shader: ShaderModule,
}
...
struct DensityErrorPushConstants {
...
impl DensityErrorCompute {
    pub fn new(vk_context: &Arc<VulkanContext>) -> anyhow::Result<Self> {
        ...
        Ok(Self {
            density_error_compute_pass,
            density_error_compute_shader,
        })
    }
    ...
    pub fn execute(
        ...
        let push_constants = DensityErrorPushConstants {
            num_elements,
