...
use crate::backends::vulkan::particles::physics::octree::octree_linker::OctreeLinker;
use crate::backends::vulkan::particles::physics::octree::rebalancer::Rebalancer;
use crate::backends::vulkan::particles::physics::octree::rebalancing_ops_marker::RebalancingOpsMarker;
...
impl OctreeConstructor {
    ...
    pub fn build(
        ...
        for _ in 0..max_levels {
            ...
            cmd_buffer.pipeline_memory_barrier(
                ...
                &[
                    barrier_compute_to_compute(
                        octree_data.rebalance_ops().vk_buffer(),
                        vk::WHOLE_SIZE,
                        vk::AccessFlags2::SHADER_STORAGE_READ,
                    ),
                    barrier_compute_to_compute(
            ...
            cmd_buffer.pipeline_memory_barrier(
                device,
                &[
                    barrier_compute_to_indirect(
                        octree_data.indirect_dispatch_buffer_leaves().vk_buffer(),
                        vk::WHOLE_SIZE,
                    ),
                    barrier_compute_to_compute(
                        octree_data.leaf_count().vk_buffer(),
                        vk::WHOLE_SIZE,
                        vk::AccessFlags2::SHADER_STORAGE_READ
                            | vk::AccessFlags2::SHADER_STORAGE_WRITE,
                    ),
                    barrier_compute_to_compute(
                        octree_data.cornerstone_array().vk_buffer(),
                        vk::WHOLE_SIZE,
                        vk::AccessFlags2::SHADER_STORAGE_READ
                            | vk::AccessFlags2::SHADER_STORAGE_WRITE,
                    ),
    ...
    fn build_internal_nodes(
        ...
        cmd_buffer.pipeline_memory_barrier(
            ...
            &[
                barrier_compute_to_compute(
                    octree_data.node_keys().vk_buffer(),
                    vk::WHOLE_SIZE,
                    vk::AccessFlags2::SHADER_STORAGE_READ,
                ),
                barrier_compute_to_compute(
                    octree_data.node_count().vk_buffer(),
                    vk::WHOLE_SIZE,
                    vk::AccessFlags2::SHADER_STORAGE_READ,
                ),
                barrier_compute_to_compute(
                    octree_data.leaf_particles().vk_buffer(),
                    vk::WHOLE_SIZE,
                    vk::AccessFlags2::SHADER_STORAGE_READ,
                ),
                barrier_compute_to_compute(
                    octree_data.unsorted_leaf_particles().vk_buffer(),
                    vk::WHOLE_SIZE,
                    vk::AccessFlags2::SHADER_STORAGE_READ,
                ),
            ],
        ...
        cmd_buffer.pipeline_memory_barrier(
            ...
            &[
                barrier_compute_to_compute(
                    octree_data.level_offsets().vk_buffer(),
                    vk::WHOLE_SIZE,
                    vk::AccessFlags2::SHADER_STORAGE_READ,
                ),
                barrier_compute_to_indirect(
                    octree_data.indirect_dispatch_buffer_nodes().vk_buffer(),
                    vk::WHOLE_SIZE,
                ),
            ],
            &[],
        ...
        cmd_buffer.pipeline_memory_barrier(
            ...
            &[
                barrier_compute_to_compute(
                    octree_data.node_first_child().vk_buffer(),
                    vk::WHOLE_SIZE,
                    vk::AccessFlags2::SHADER_STORAGE_READ,
                ),
                barrier_compute_to_compute(
                    octree_data.node_bounding_boxes().vk_buffer(),
                    vk::WHOLE_SIZE,
                    vk::AccessFlags2::SHADER_STORAGE_READ,
                ),
            ],
