...
impl OctreeData {
    pub fn new(
        ...
        let cornerstone_array: PingPong<VkBuffer<u32>> = PingPong::new_vk_buffer(
            vk_context,
            &cornerstone_initial_data,
            "Cornerstone array",
            cmd_pool,
            queue,
        )?;
        ...
        let rebalance_prefix = VkBuffer::new_gpu_only(
            vk_context,
            &vec![0 as u32; (max_leaves + 1) as usize],
            "Rebalance prefix",
            cmd_pool,
            queue,
        )?;

        let node_keys = VkBuffer::new_gpu_only(
            vk_context,
            &vec![0xffffffff as u32; total_nodes as usize],
            "node_keys",
            cmd_pool,
            queue,
        )?;
        let node_first_child = VkBuffer::new_gpu_only(
            vk_context,
            &vec![0 as u32; total_nodes as usize],
            "node_first_child",
            cmd_pool,
            queue,
        )?;
        let level_offsets = VkBuffer::new_gpu_only(
            vk_context,
            &vec![0 as u32; (max_levels + 2) as usize],
            "level_offsets",
            cmd_pool,
            queue,
        )?;

        let leaf_offsets = VkBuffer::new_gpu_only(
            vk_context,
            &vec![0 as u32; (max_leaves + 1) as usize],
            "level_offsets",
            cmd_pool,
        ...
        let leaf_particles = VkBuffer::new_gpu_only(
            vk_context,
            &vec![LeafParticles::default(); total_nodes as usize],
            "leaf_particles",
            cmd_pool,
            queue,
        )?;
        let unsorted_leaf_particles = VkBuffer::new_gpu_only(
            vk_context,
            &vec![LeafParticles::default(); total_nodes as usize],
        ...
        let leaf_count = PingPong::new_vk_buffer(
            vk_context,
            &vec![1 as u32; 1 as usize],
            "leaf_count",
            cmd_pool,
            queue,
        )?;
        let node_count = VkBuffer::new_gpu_only(
            vk_context,
            &vec![1 as u32; 1 as usize],
            "node_count",
            cmd_pool,
        ...
        let node_bounding_boxes = VkBuffer::new_gpu_only_uninitialized(
            vk_context,
            total_nodes as usize,
            "Node bounding boxes",
        )?;

        let was_changed = VkBuffer::new_gpu_only(
            vk_context,
            &vec![0 as u32; 1 as usize],
            "was changed",
            cmd_pool,
            queue,
        )?;

        let indirect_dispatch_buffer_leaves = IndirectBuffer::new(
            vk_context,
            cmd_pool,
            &[glam::UVec4::new(1, 1, 1, 0), glam::UVec4::new(1, 1, 1, 0)],
        )?;
        let indirect_dispatch_buffer_nodes =
            IndirectBuffer::new(vk_context, cmd_pool, &[glam::UVec4::new(1, 1, 1, 0)])?;
