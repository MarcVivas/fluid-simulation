...
impl ParticleSolver {
    pub fn new(
        ...
        let sorter = GpuKVRadixSort::new(vk_context, cmd_pool, max_objects, None)?;
        let particle_reorderer = ParticleReorderer::new(vk_context)?;
        let hilbert_encoder = HilbertEncoder::new(vk_context, max_levels)?;

        let apply_external_forces = ApplyExternalForces::new(vk_context)?;
        let density_error_compute = DensityErrorCompute::new(vk_context)?;
        let density_factor_compute = DensityFactorCompute::new(vk_context)?;
        let divergence_compute = DivergenceCompute::new(vk_context)?;
        let integrate_positions = IntegratePositions::new(vk_context)?;
        let pressure_velocity_update = PressureVelocityUpdate::new(vk_context)?;
    ...
    pub fn update(
        ...
        gpu_profiler.profile_scope(
            ...
            || {
                self.particle_reorderer
                    .execute(vk_context, particle_data, command_buffer);
            },
        );
        ...
        gpu_profiler.profile_scope(device, vk_cmd_buffer, "Density factor compute", ring_idx, ||{
            self.density_factor_compute.execute(vk_context, command_buffer, neighbor_list, particles, &self.physics_config);
        });
        ...
        for i in 0..self.physics_config.dfsph.divergence_iterations {
            ...
            gpu_profiler.profile_scope(device, vk_cmd_buffer, &label, ring_idx, ||{
                self.divergence_compute.execute(vk_context, command_buffer, neighbor_list, particles, &self.physics_config);
            });
            ...
            gpu_profiler.profile_scope(device, vk_cmd_buffer, &label_pressure, ring_idx, ||{
                self.pressure_velocity_update.execute(vk_context, command_buffer, neighbor_list, particles, &self.physics_config);
            });
        ...
        gpu_profiler.profile_scope(device, vk_cmd_buffer, "Apply external forces", ring_idx, ||{
            self.apply_external_forces.execute(vk_context, command_buffer, particles, &self.physics_config, &world_bounds);
        });

        for i in 0..self.physics_config.dfsph.density_iterations {
            let label = format!("Density error compute {}", i);
            gpu_profiler.profile_scope(device, vk_cmd_buffer, &label, ring_idx, ||{
                self.density_error_compute.execute(vk_context, command_buffer, neighbor_list, particles, &self.physics_config);
            });
        ...
        gpu_profiler.profile_scope(device, vk_cmd_buffer, "Integrate", ring_idx, ||{
            self.integrate_positions.execute(vk_context, command_buffer, particles, &self.physics_config, &world_bounds);
        });
    }
}
