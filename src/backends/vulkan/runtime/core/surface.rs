...
impl Surface {
    pub fn new(entry: &Entry, instance: &Instance, window: &Window) -> anyhow::Result<Surface> {
        let surface = unsafe {
    ...
    pub fn surface_resolution(
        &self,
        physical_device: PhysicalDevice,
        window: &Window,
    ) -> anyhow::Result<Extent2D> {
        let surface_capabilities =
            self.get_physical_device_surface_capabilities(physical_device)?;
