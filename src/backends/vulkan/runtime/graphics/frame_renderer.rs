...
use crate::backends::vulkan::runtime::frame::frame_data::FrameData;
...
use crate::backends::vulkan::runtime::swapchain::swapchain_presenter::SwapchainPresenter;
...
impl FrameRenderer {
    ...
    pub fn submit_and_present(
        &mut self,
        external_waits: &[vk::SemaphoreSubmitInfo],
        frame_pacer: &FramePacer,
        image_index: u32,
    ) -> Result<()> {
        let ring_idx = frame_pacer.ring_index();
        ...
        wait_semaphores.extend_from_slice(external_waits);
    ...
    pub fn previous_frame_wait(
        ...
        let previous_value = frame_pacer.total_frames_processed();
        ...
        Some(
            self.frame_synchronizer
                .timeline_semaphore()
                .wait_info(previous_value, vk::PipelineStageFlags2::ALL_COMMANDS),
        )
    }
}
