...
impl ComputePass {
    ...
    fn set_push_constants(&self, device: &ash::Device, cmd_buffer: &CommandBuffer, push_constants: &[u8]){
        ...
        if !descriptor_writes.is_empty() {
            unsafe {
                vk_core.push_descriptor().cmd_push_descriptor_set(
                    cmd_buffer.vk_cmd_buffer(), vk::PipelineBindPoint::COMPUTE,
                    self.pipeline_layout.vk_pipeline_layout(),
                    0,
                    &descriptor_writes
                );
