...
use crate::vk_utils::allocation::{allocate, deallocate};
...
impl AllocatedImage {
    ...
    {
        let image = unsafe {
            vk_core.device().create_image(&image_create_info, None)
        }?;

        let image_memory_requirements = unsafe {
            vk_core.device().get_image_memory_requirements(image)
        };
        ...
        unsafe {
            vk_core.device().bind_image_memory(
                image,
                allocation.memory(),
                allocation.offset(),
            )
