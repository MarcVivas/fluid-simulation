...
impl TimelineSemaphore {
    pub fn new(vk_core: Arc<VulkanContext>, initial_value: u64) -> Result<Self, vk::Result> {
        ...
        let semaphore = unsafe {
            vk_core.device().create_semaphore(&semaphore_info, None)?
        };
    ...
    pub fn wait_on_host(&self, value: u64, timeout_ns: u64) -> VkResult<()>{
        ...
        unsafe {
            self.vk_core.device().wait_semaphores(&wait_info, timeout_ns)
        }
    }

    /// Generates a submit info struct to signal this semaphore at a specific stage.
    pub fn signal_info(&self, value: u64, stage_mask: vk::PipelineStageFlags2) -> vk::SemaphoreSubmitInfo<'static> {
    ...
    pub fn wait_info(&self, value: u64, stage_mask: vk::PipelineStageFlags2) -> vk::SemaphoreSubmitInfo<'static>  {
        vk::SemaphoreSubmitInfo::default()
            .semaphore(self.semaphore)
            .value(value)
            .stage_mask(stage_mask)
