...
fn validate_ids(result_point_ids: &Vec<u32>, count: usize){
    for i in 0..count {
        assert_eq!(result_point_ids[i], i as u32, "Object ID mapping was corrupted at index {}", i);
    }
}
...
fn validate_determinism(result_keys: &Vec<u32>){
    ...
    assert_eq!(
        ...
        "Determinism failed! Identical points produced different keys: {} vs {}", result_keys[0], result_keys[1]
    );
}
...
fn generate_test_data(
    ...
    let duplicate_point = Vec4::new(world_min.x + 10.0, world_min.y + 15.0, world_min.z + 20.0, 1.0);
    input_points.push(duplicate_point);
    input_points.push(duplicate_point);

    // 2. Inject Out-of-Bounds Test Points (Indices 2 and 3)
    input_points.push(Vec4::new(world_min.x - 5000.0, world_min.y - 5000.0, world_min.z - 5000.0, 1.0)); // Way below min
    input_points.push(Vec4::new(world_min.x + 5000.0, world_min.y + 5000.0, world_min.z + 5000.0, 1.0)); // Way above max
